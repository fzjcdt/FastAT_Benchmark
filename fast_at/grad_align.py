...
class GradAlign(ATBase):

    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='grad_align', device=None, seed=0):
        super(GradAlign, self).__init__(model, config_path=config_path, config=config, 
                                      log_dir=log_dir, name=name, device=device, seed=seed)
        self.grad_align_cos_lambda = self.config.get('grad_align_cos_lambda', 0.2)
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        self.logger.log('grad_align_cos_lambda: {}'.format(self.grad_align_cos_lambda))
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                # alpha =  eps
                adv_images = adv_images.detach() + self.eps * torch.sign(grad)
                ...
                reg = torch.zeros(1).to(self.device)[0]  # for .item() to run correctly

                adv_images_uniform = images.clone() + torch.zeros_like(images).uniform_(-self.eps, self.eps)
                adv_images_uniform = torch.clamp(adv_images_uniform, min=0, max=1)
                adv_images_uniform = adv_images_uniform.detach().requires_grad_(True)
                output_uniform = self.model(adv_images_uniform)
                loss_uniform = F.cross_entropy(output_uniform, labels)
                grad2 = torch.autograd.grad(loss_uniform, adv_images_uniform, create_graph=True)[0]
