...
class FGSM_PGI(ATBase):

    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='fgsm_pgi', device=None, seed=0):
        super(FGSM_PGI, self).__init__(model, config_path=config_path, config=config,
                                       log_dir=log_dir, name=name, device=device, seed=seed)
        self.epochs_reset = self.config.get('epochs_reset', 40)
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        self.logger.log('epochs_reset: {}'.format(self.epochs_reset))
        ...
        for epoch in range(total_epoch):
            ...
            if epoch % self.epochs_reset == 0:
                all_delta = torch.zeros(training_shape).uniform_(-self.eps, self.eps).to(self.device)
                all_delta = torch.clamp(self.eps * torch.sign(all_delta), -self.eps, self.eps)
                all_momentum = torch.zeros(training_shape).to(self.device)

            for idx, images, labels in train_loader:
                images, labels = images.to(self.device), labels.to(self.device)
                # add uniform noise
                # noise magnitude k = 2 * eps
                delta = all_delta[idx].clone().detach()
                momentum = all_momentum[idx].clone().detach()

                adv_images = torch.clamp(images.clone() + delta, 0, 1)
                ...
                x_grad = adv_images.grad.detach()
                grad_norm = torch.norm(x_grad, p=1)
                momentum = x_grad / grad_norm + momentum * self.momentum_decay

                next_delta = torch.clamp(delta + self.eps * torch.sign(momentum), -self.eps, self.eps)
                next_delta = torch.clamp(next_delta, -images, 1 - images)

                delta = torch.clamp(delta + self.eps * torch.sign(x_grad), -self.eps, self.eps)
                delta = torch.clamp(delta, -images, 1 - images)
                adv_images = images.clone() + delta
                ...
                all_momentum[idx] = momentum.detach()
            ...
            if weight_average:
                ...
                self.logger.log(
                    'WA Model - Validation accuracy: {:.4f}'.format(validation_results['wa_model']['val_acc']))
