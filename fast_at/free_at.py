...
class Free_AT(ATBase):

    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='free_at', device=None, seed=0):
        super(Free_AT, self).__init__(model, config_path=config_path, config=config, 
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        total_epoch = self.config.get('total_epoch', 200)
        ...
        self.logger.log('m: {}'.format(self.m))
        ...
        for epoch in range(total_epoch // self.m):
