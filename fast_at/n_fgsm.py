...
class NFGSM(ATBase):
    ...
    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='n_fgsm', device=None, seed=0):
        super(NFGSM, self).__init__(model, config_path=config_path, config=config, 
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                adv_images = images.clone() + torch.zeros_like(images).uniform_(-2 * self.eps, 2 * self.eps)
