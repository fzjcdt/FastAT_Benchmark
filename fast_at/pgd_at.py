...
class PGD_AT(ATBase):
    ...
    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='pgd_at', device=None, seed=0):
        super(PGD_AT, self).__init__(model, config_path=config_path, config=config, log_dir=log_dir, name=name,
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        pgd_adv_images_generator = PGD(self.model, eps=self.eps, alpha=2.0 / 255, steps=10)
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                adv_images = pgd_adv_images_generator(images, labels)
