...
class Nu_AT(ATBase):
    ...
    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='nu_at', device=None, seed=0, nuc_max=None):
        super(Nu_AT, self).__init__(model, config_path=config_path, config=config, log_dir=log_dir, name=name, device=device, seed=seed)
        self.nuc_max = nuc_max or self.config.get('nuc_max', 4.0)
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        best_pgd_acc, best_val_acc, total_training_time, total_val_time = 0.0, 0.0, 0.0, 0.0
        train_acc_list = []
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                # self.model.eval()
                # opt.zero_grad()
                rout = self.model(adv_images)

                if counter % 2 == 1:
                    loss = criterion(rout, labels) + nuc_reg * torch.norm(output - rout, 'nuc') / labels.size(0)
                else:
                    loss = criterion(rout, labels)
                ...
                reg_loss = torch.norm(output - adv_output, 'nuc') / labels.size(0)
                loss = closs + nuc_reg * reg_loss
            ...
            scheduler.step()
            nuc_reg += self.nuc_max / total_epoch
