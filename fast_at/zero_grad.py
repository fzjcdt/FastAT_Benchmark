...
class ZeroGrad(ATBase):

    def __init__(self, model, config_path=None, config=None, log_dir='./log/', name='zero_grad', device=None, seed=0):
        super(ZeroGrad, self).__init__(model, config_path=config_path, config=config, 
                                       log_dir=log_dir, name=name, device=device, seed=seed)
        self.q_val = self.config.get('q_val', 0.35)
        self.q_iters = self.config.get('q_iters', 1)
        self.fgsm_init = self.config.get('fgsm_init', 'random')
        self.fgsm_alpha = self.config.get('fgsm_alpha', 2.0)

    def _zero_grad_attack(self, X, y, epsilon, alpha, q_val, q_iters, fgsm_init):
        ...
        for i in range(q_iters):
            ...
            CrossEntropyLoss()(output, y).backward()
    ...
    def train(self, opt, scheduler, train_loader, val_loader, test_loader=None):
        ...
        self.logger.log('q_val: {}'.format(self.q_val))
        self.logger.log('q_iters: {}'.format(self.q_iters))
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                delta = self._zero_grad_attack(
                    images, labels, self.eps, self.fgsm_alpha * self.eps, 
                    self.q_val, self.q_iters, self.fgsm_init
                )
